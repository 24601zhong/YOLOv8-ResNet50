"""
============================================================
ArcFace 人脸识别 1:1 验证集评估
数据集: insightface 官方 eval/*.bin (pickle: [jpeg_bytes列表, issame布尔列表])
...
评估: 提取 512 维 embedding → 1:1 余弦相似度 → 网格搜索最优阈值 → accuracy
(可选 --flip: 测试时镜像增强 + 特征平均, 通常 +0.1~0.2%)

用法:
  python face_arcface/eval_face_verification.py --model face_arcface/output/last.pt
  python face_arcface/eval_face_verification.py --model .../epoch_20.pt --flip
============================================================
"""
import os
import sys
...
def main():
    ...
    EVAL_DIR = arg('--data', 'dataset/archive/eval')
